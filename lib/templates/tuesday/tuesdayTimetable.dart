...
class _TuesdayState extends State<Tuesday> {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Tuesday', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold),),
          centerTitle: true,
        ),
        body: Container(
                  ...
                  itemBuilder: (BuildContext context, int index) =>
                      Slidable(
                        ...
                        actionExtentRatio: 0.20,
                          child: TuesdayCard(context, snapshot.data.documents[index]),
                        actions: <Widget>[
                          IconSlideAction(
                            caption: 'Delete',
                            color: Colors.red,
        ...
        floatingActionButton: FloatingActionButton.extended(
          onPressed: (() =>
            Navigator.of(context).pushNamed('/addTuesdayTimetable'))
          ,
  ...
  Stream<QuerySnapshot> getUserTimetableSnapshot(BuildContext context) async* {
    ...
    yield* Firestore.instance.collection('userData').document(uid).collection('tuesday').snapshots();
  }

  Widget TuesdayCard(BuildContext context, DocumentSnapshot tuesday) {
    ...
    return Container(
      ...
      child: Card(
        ...
        child: Padding(
          ...
          child: Column(
            children: <Widget>[
              Align(alignment: Alignment.topLeft,child: Text('${tuesday['startTime']} - ${tuesday['endTime']}', style: TextStyle(fontSize: 25.0, fontWeight: FontWeight.w300, color: boxColor),)),
              SizedBox(height: 30.0,),
              Align(alignment: Alignment.center,child: Text(tuesday['course'], style: TextStyle(fontSize: 20.0, fontWeight: FontWeight.bold,color: boxColor),)),
