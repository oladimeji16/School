...
class AddMondayTimetable extends StatefulWidget {
  @override
  _AddMondayTimetableState createState() => _AddMondayTimetableState();
}

class _AddMondayTimetableState extends State<AddMondayTimetable> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Add a New Course', style: TextStyle(fontSize: 22.0, color: boxColor, fontWeight: FontWeight.bold,),),
        centerTitle: true,
      ),
      body: Center(
        child: Form(
                    ...
                    onPressed: () async {
                      ...
                      if(_formKey.currentState.validate()) {
                        try{
                          ...
                          await db.collection('userData').document(uid).collection('monday').add(data.toJson());
                          Navigator.pop(context);
                        } catch(e){
                          setState(() {
                            Dialog alert = Dialog(
                              child: Text('Unable to complete your request at this time, \n please try again'),
                            );
